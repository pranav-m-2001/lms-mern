...
function Register(){
    ...
    const { backenUrl,navigate } = useContext(AppContext)
    ...
    async function handleSubmit(event){
        ...
        try{
            ...
            const response = await axios.post(`${backenUrl}/api/user/signup`, {name,email,password}, {withCredentials: true})
    ...
    return(
        ...
        <input className="w-full px-3 py-2 border border-gray-800 outline-blue-300" type="text" placeholder="Name" value={name} onChange={(event)=>setName(event.target.value)} required />
        ...
        <div className="w-full flex justify-between text-sm mt-[-8px]">
            ...
            <p  className="cursor-pointer text-blue-600">Login</p>
        </div>
        <button className="bg-blue-600 w-full rounded-md text-white font-medium px-8 py-2 mt-2 cursor-pointer">{ loading ? 
